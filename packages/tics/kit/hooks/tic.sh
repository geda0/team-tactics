...
# Kinds: delegate handoff signal block stuck verdict msg note claim release contract need section commit.
set -uo pipefail
...
case "${3:-}" in
  ...
  log|inbox|conductor|claims|sections|cycle|report|-*)
    ...
    exit 2 ;;
  *)
    echo "tic.sh: '${3:-}' is not a tic kind. Valid: delegate handoff signal block stuck verdict msg note claim release contract need section commit. Nothing recorded. Usage: tic.sh FROM TO KIND MSG [REF] [RESULT]" >&2
    exit 2 ;;
esac
