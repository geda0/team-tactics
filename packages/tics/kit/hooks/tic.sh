...
# Kinds: delegate handoff signal block verdict msg note claim release contract need section commit.
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; ROOT="$(cd "$HERE/../.." && pwd)"
# shellcheck disable=SC1091
. "$ROOT/.claude/hooks/tics-lib.sh"
case "${3:-}" in
  delegate|handoff|signal|block|verdict|msg|note|claim|release|contract|need|section|commit)
    emit_tic "$@" ;;
  log|inbox|conductor|claims|sections|cycle|report|-*)
    ...
    exit 2 ;;
  *)
    echo "tic.sh: '${3:-}' is not a tic kind. Valid: delegate handoff signal block verdict msg note claim release contract need section commit. Nothing recorded. Usage: tic.sh FROM TO KIND MSG [REF] [RESULT]" >&2
    exit 2 ;;
esac
