...
- `section` — the lifecycle of a partition (the scope's first component, `ref`). **Opening is automatic:** the guard auto-opens a section on first scoped edit, so the partition map fills itself — mark it shipped with `tic.sh <pair> '*' section done <name> done`. `tics sections` is the live map (each section's `[open|active|done]` status + tics/claims/contracts/needs); `tics section-status <name>` reports one section's latest status (empty if unopened).
